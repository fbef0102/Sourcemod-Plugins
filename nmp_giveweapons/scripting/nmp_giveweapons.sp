...
public OnPluginStart()
{
	...
	nmp_giveweapons_version = CreateConVar( "nmp_giveweapons_version", PLUGIN_VERSION, "NoMorePlugins Give Weapons", FCVAR_REPLICATED|FCVAR_SPONLY|FCVAR_DONTRECORD );
	SetConVarString( nmp_giveweapons_version, PLUGIN_VERSION );
	HookConVarChange( nmp_giveweapons_version, OnConVarChanged_Version );
	
	HookConVarChange( nmp_gw_silent = CreateConVar( "nmp_gw_silent", bSilentAdmin ? "1" : "0", "Hide admin activity.", FCVAR_NOTIFY, true, 0.0, true, 1.0 ), OnConVarChanged );
...
public OnLibraryRemoved( const char[] szLibName )
...
public Handle_MenuCategory( Handle:hMenu, TopMenuAction:iAction, TopMenuObject:iObject, iParam, String:szBuffer[], iBufferLength )
	switch( iAction )
	{
		case TopMenuAction_DisplayOption:
			Format( szBuffer, iBufferLength, "%T", "NMP AM Category", iParam );
		case TopMenuAction_DisplayTitle:
			Format( szBuffer, iBufferLength, "%T", "NMP AM Action", iParam );
	}
#endif
public Handle_MenuGiveItem( Handle:hMenu, TopMenuAction:iAction, TopMenuObject:iObject, iParam, String:szBuffer[], iBufferLength )
	...
	{
		case TopMenuAction_DisplayOption:
			Format( szBuffer, iBufferLength, "%T", "NMP AM Give Item", iParam );
		case TopMenuAction_SelectOption:
		{
			g_iSelectMenuPos[iParam] = 0;
...
public Handle_MenuSpawnItem( Handle:hMenu, TopMenuAction:iAction, TopMenuObject:iObject, iParam, String:szBuffer[], iBufferLength )
	...
	{
		case TopMenuAction_DisplayOption:
			Format( szBuffer, iBufferLength, "%T", "NMP AM Spawn Item", iParam );
		case TopMenuAction_SelectOption:
		{
			g_iSelectMenuPos[iParam] = 0;
...
stock ShowAdminMenu( iClient, nType )
{
	...
	else
	{
		...
		SetMenuExitButton( hMenu, true );
		SetMenuExitBackButton( hMenu, true );
		DisplayMenuAtItem( hMenu, iClient, g_iSelectMenuPos[iClient], MENU_TIME_FOREVER );
	}
}
...
public Menu_GiveItem( Handle:hMenu, MenuAction:iAction, iParam1, iParam2 )
	switch( iAction )
	{
		...
		{
			g_iSelectMenuPos[iParam1] = GetMenuSelectionPosition();
