...
public void OnPluginStart()
{
	...
	cVarMinLerp 		= CreateConVar("sm_min_lerp", 		"0.000", 	"Minimum allowed lerp value, Force player to Spec if lower than Minimum", CVAR_FLAGS);
	cVarMaxLerp 		= CreateConVar("sm_max_lerp", 		"0.1", 		"Maximum allowed lerp value, Force player to Spec if greater than Maximum", CVAR_FLAGS);
...
Action COLDOWN(Handle timer, any client)
{
...
Action OnTeamChangeDelay(Handle timer, any client)
{
	int iTeam;
...
{	
	...
	if ( ((FloatCompare(m_fLerpTime, cVarMinLerp.FloatValue) == -1) || (FloatCompare(m_fLerpTime, cVarMaxLerp.FloatValue) == 1)) && GetClientTeam(client) != 1) {
		...
		CPrintToChatAll("<{olive}Lerp{default}> {lightgreen}%N{default}'s Lerp {olive}%.01f{default}, moved to spec!", client, m_fLerpTime*1000);
