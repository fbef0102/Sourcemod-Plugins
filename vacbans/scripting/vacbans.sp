...
//SteamWorks dead in l4d1
//#tryinclude <SteamWorks> //https://github.com/hexa-core-eu/SteamWorks/releases
//#tryinclude <steamtools> //https://builds.limetech.io/?p=steamtools
#tryinclude <socket> //https://github.com/JoinedSenses/sm-ext-socket/releases
...
// 去除https
// 配色
// 移除steamworks改用socket (L4D1無法用steamworks)
#define PLUGIN_VERSION "1.0h-2025/10/5"
...
char g_sCVImmunity[AdminFlags_TOTAL];
...
public void OnPluginStart()
{
	...
	g_hCVImmunity  = CreateConVar( "sm_vacbans_immunity",   "z",    "Players with these flags will be ignored (Empty = Everyone, -1: Nobody)", FCVAR_NOTIFY);
	...
	g_hCVImmunity.AddChangeHook(OnConVarChanged);

	Format(desc, sizeof(desc), "%T", "Command_Reset", LANG_SERVER);
	RegAdminCmd("sm_vacbans_reset", Command_Reset, ADMFLAG_ROOT, desc);

	Format(desc, sizeof(desc), "%T", "Command_Whitelist", LANG_SERVER);
	RegAdminCmd("sm_vacbans_whitelist", Command_Whitelist, ADMFLAG_ROOT, desc);

	Format(desc, sizeof(desc), "%T", "Command_List", LANG_SERVER);
	RegAdminCmd("sm_vacbans_list", Command_List, ADMFLAG_ROOT, desc);
...
void OnDBConVarChanged(ConVar convar, const char[] oldValue, const char[] newValue)
{
...
void GetCvars()
{
	g_hCVImmunity.GetString(g_sCVImmunity, sizeof(g_sCVImmunity));
}
...
public void OnClientPostAdminCheck(int client)
{
	...
	{
		if (HasAccessWithoutRoot(client, g_sCVImmunity))
		{
#if DEBUG
...
Action Command_Reset(int client, int args)
{
...
Action Command_Whitelist(int client, int args)
{
	char argString[72];
	char action[8];
...
Action Command_List(int client, int args)
{
	...
	if (IsClientAuthorized(client))
	{
		...
		if (vacBanned || gameBanned || commBanned || econBanned || econProbation)
		{
			...
			if (actions & ACTION_BAN)
			{
				...
				ServerCommand("sm_ban #%d %d \"[VAC Status Checker] %T\"", GetClientUserId(client), 365*24*60, "Player_Message", client, "Banned", reason);
			}
			...
			if (actions & ACTION_NOTIFY_ALL || actions & ACTION_NOTIFY_ADMINS)
			{
				for (int i = 1; i <= MaxClients; i++)
				{
					...
					CPrintToChat(i, "[{green}VAC Status Checker{default}] {olive}%N{default} (%t)", client, "Admin_Message2", numVACBans, numGameBans, commStatusText, econStatusText);
				}
			}
		}
...
void OnQueryVersionCheck(Database db, DBResultSet results, const char[] error, any data)
{
...
void OnQueryVersionCreated(Database db, DBResultSet results, const char[] error, any data)
{
	char query[64];
...
void OnQueryCacheCreated(Database db, DBResultSet results, const char[] error, any data)
{
	if (results != null)
	{
...
void OnQueryMigrate(Database db, DBResultSet results, const char[] error, any data)
{
	if (results != null)
	{
		char steamId[18];
...
void OnQueryPlayerLookup(Database db, DBResultSet results, const char[] error, DataPack data)
{
	bool checked = false;
...
void OnQueryNoOp(Database db, DBResultSet results, const char[] error, any data)
{
	// Nothing to do
}
